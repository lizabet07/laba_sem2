...
class BankAccount:
    ...
    def deposit(self, amount):
        """Пополнение счёта"""
        # Проверка статуса
        is_valid, error = validate.validate_status_for_operation(self._status, "пополнить счёт")
        if not is_valid:
    ...
    def calculate_interest(self):
        """Расчёт процентов (бизнес‑метод 1)"""
    ...
    def apply_interest(self):
        ...
        if self._status == "закрыт":
            raise PermissionError("Нельзя начислить проценты на закрытый счёт")
    ...
    def block(self):
        """Заблокировать счёт"""
        if self._status == "закрыт":
            raise PermissionError("Нельзя заблокировать закрытый счёт")
        self._status = "заблокирован"

    def activate(self):
        """Активировать счёт"""
        if self._status == "закрыт":
            raise PermissionError("Нельзя активировать закрытый счёт")
    ...
    def transfer_to(self, other_account, amount):
        ...
        # Снимаем с этого счёта
        self.withdraw(amount)

        try:
            # Кладём на другой счёт
